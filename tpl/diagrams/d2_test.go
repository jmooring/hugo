...
func Test_sanitizeSVG(t *testing.T) {
	testCases := []struct {
		input    []byte
		expected []byte
	}{
		{
			input:    []byte(`<svg id="d2-svg">...</svg>`),
			expected: []byte(`<svg>...</svg>`),
		},
		{
			input:    []byte(`<svg id="d2-svg" foo="bar">...</svg>`),
			expected: []byte(`<svg foo="bar">...</svg>`),
		},
		{
			input:    []byte(`<svg>...</svg>`),
			expected: []byte(`<svg>...</svg>`),
