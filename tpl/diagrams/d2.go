...
func (d d2SVG) String() string {
	return fmt.Sprintf(
		`<svg xmlns=%q xmlns:xlink=%q viewBox=%q width="%d" height="%d" preserveAspectRatio=%q>%s</svg>`,
...
func createD2SVG(markup string, opts *d2Options) (*d2SVG, error) {
	...
	layoutResolver := func(engine string) (d2graph.LayoutGraph, error) {
		...
		default:
			return nil, errors.New("layout engine must be one of elk, dagre, or tala")
		}
	}
...
func validateOptions(opts *d2Options) error {
	...
	if opts.LayoutEngine != "dagre" && opts.LayoutEngine != "elk" && opts.LayoutEngine != "tala" {
		return errors.New("layout engine must be one of elk, dagre, or tala")
	}
	if opts.LightTheme == "" {
